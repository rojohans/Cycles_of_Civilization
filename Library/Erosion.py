...
class HydrolicErosion():
    ...
    def Rain(self, numberOfDrops = 1, radius = 1, dropSize = 1, application = 'drop', rainMap = None):
        if application == 'drop':
        ...
        elif application == 'map':
            self.water[:, :, 0] += rainMap
    ...
    def Visualize(self):
        if self.terrainplot == None:
            if False:
                ...
                self.sedimentplot = axs[1][1].imshow(self.terrain[:, :, -1])
...
class HydrolicErosionScaled():
    ...
    def Rain(self, numberOfDrops = 1, radius = 1, dropSize = 1, application = 'drop', rainMap = None):
        '''
        The area of the tile has to be taken into consideration, thusly deltaLatitude and deltaLongitude is used.
        :param numberOfDrops:
        :param radius:
        :param dropSize:
        :param application:
    ...
    def UpdateFlow(self):
        deltaHeightLeft = self.CalculateDeltaHeight('left')
        #newFlowLeft = self.flow[:, :, 0] + self.deltaT * self.flowSpeed * deltaHeightLeft
        #newFlowLeft = self.flow[:, :, 0] + self.deltaT*self.flowSpeed*deltaHeightLeft*self.deltaLatitude
        newFlowLeft = self.flow[:, :, 0] + self.deltaT*self.flowSpeed*deltaHeightLeft/self.deltaLongitude
        newFlowLeft[newFlowLeft < 0] = 0

        deltaHeightRight = self.CalculateDeltaHeight('right')
        #newFlowRight = self.flow[:, :, 1] + self.deltaT * self.flowSpeed * deltaHeightRight
        #newFlowRight = self.flow[:, :, 1] + self.deltaT*self.flowSpeed*deltaHeightRight*self.deltaLatitude
        newFlowRight = self.flow[:, :, 1] + self.deltaT*self.flowSpeed*deltaHeightRight/self.deltaLongitude
        newFlowRight[newFlowRight < 0] = 0

        deltaHeightTop = self.CalculateDeltaHeight('top')
        #newFlowTop = self.flow[:, :, 2] + self.deltaT * self.flowSpeed * deltaHeightTop
        #newFlowTop = self.flow[:, :, 2] + self.deltaT*self.flowSpeed*deltaHeightTop*self.deltaLongitude
        newFlowTop = self.flow[:, :, 2] + self.deltaT*self.flowSpeed*deltaHeightTop/self.deltaLatitude
        newFlowTop[newFlowTop < 0] = 0

        deltaHeightBottom = self.CalculateDeltaHeight('bottom')
        #newFlowBottom = self.flow[:, :, 3] + self.deltaT * self.flowSpeed * deltaHeightBottom
        #newFlowBottom = self.flow[:, :, 3] + self.deltaT*self.flowSpeed*deltaHeightBottom*self.deltaLongitude
        newFlowBottom = self.flow[:, :, 3] + self.deltaT*self.flowSpeed*deltaHeightBottom/self.deltaLatitude
        newFlowBottom[newFlowBottom < 0] = 0
        ...
        # The 0.9 is to reduce instability in the flow simulation.
        flowScaling = self.water[:, :, 0]*self.deltaLatitude*self.deltaLongitude/((self.flow[:, :, 0] + self.flow[:, :, 1] + self.flow[:, :, 2] + self.flow[:, :, 3]+0.001)*self.deltaT)
        flowScaling[flowScaling>1] = 1
        #flowScaling[flowScaling > 0.5] = 0.5
        ...
        self.inFlowTop[0:self.NRows-1, :] = self.flow[1:self.NRows, :, 3]
        self.inFlowTop[self.NRows-1:self.NRows, :] = np.roll(self.flow[self.NRows-1:self.NRows, :, 2], shift=int(self.layerShape[1]/2), axis = 1)
        #print(np.roll(self.flow[self.NRows-1:self.NRows, :, 2], shift=int(self.layerShape[1]/2), axis = 1))
        '''
        self.inFlowRight = np.concatenate((self.flow[:, 1:self.NColons, 0],
                                      self.flow[:, 0:1, 0]),
                                     axis=1)
    ...
    def CalculateDeltaHeight(self, direction = 'left'):
        ...
        if direction == 'left':
            ...
            #                                    axis=0)
            totalHeightWrapped = np.concatenate((totalHeight[1:self.NRows, :],
                                                 np.roll(totalHeight[self.NRows-1:self.NRows, :], shift=int(self.layerShape[1]/2), axis=1)),
                                                axis=0)
        elif direction == 'bottom':
            totalHeightWrapped = np.concatenate((totalHeight[0:1, :],
                                                 totalHeight[0:self.NRows-1, :]),
    ...
    def UpdateWaterHeight(self):
        self.water[:, :, 1] = self.water[:, :, 0] + self.deltaT*(- (
                self.flow[:, :, 0] + self.flow[:, :, 1] + self.flow[:, :, 2] + self.flow[:, :,3]) + \
                              self.inFlowLeft + self.inFlowRight + self.inFlowTop + self.inFlowBottom)/(self.deltaLatitude*self.deltaLongitude)
        #self.water[:, :, 1] = self.water[:, :, 0] + self.deltaT*(- (self.flow[:, :, 0] + self.flow[:, :, 1]) + self.inFlowLeft + self.inFlowRight)
        #self.water[:, :, 1] = self.water[:, :, 0] + self.deltaT * (- (self.flow[:, :, 1]) + self.inFlowLeft)
        #self.water[:, :, 1] = self.water[:, :, 0] + self.deltaT * (- (self.flow[:, :, 0]) + self.inFlowRight)
        #self.water[:, :, 1] = self.water[:, :, 0] + self.deltaT*(- (self.flow[:, :, 2] + self.flow[:, :,3]) + self.inFlowTop + self.inFlowBottom)
    ...
    def UpdateSlope(self):
        ...
        self.slope = np.sqrt(self.slope[0] ** 4 + self.slope[1] ** 4)/r
        self.slope[r == 0] = 0
        #self.slope *= self.deltaLongitude

        #plt.imshow(self.slope)
        #plt.show()
    ...
    def SedimentTransportation(self):
        ...
        flowScaling = self.suspendedSediment[:, :, 1]*self.deltaLatitude*self.deltaLongitude / ((self.sedimentFlow[:, :, 0] + self.sedimentFlow[:, :, 1] + self.sedimentFlow[:, :,
                                                                                        2] + self.sedimentFlow[:, :,
                                                                                             3] + 0.001) * self.deltaT)
        flowScaling[flowScaling > 1] = 1
        ...
        self.suspendedSediment[:, :, 0] = self.suspendedSediment[:, :, 1] + (- sedimentOut + sedimentIn)/(self.deltaLatitude*self.deltaLongitude)
        #self.water[:, :, 1] -= sedimentOut
        #self.water[:, :, 1] += sedimentIn
        '''
...
class ThermalErosionScaled():
    ...
    def UpdateFlow(self, iLayer):
        ...
        # Left, Right, Top, Bottom, Top_Left, Bottom_Right, Bottom_Left, Top_Right
        self.CalculateDeltaHeight(iLayer)
        self.flow0 = self.deltaT*self.flowSpeed*(self.deltaHeight0 - self.maximumDeltaHeight[iLayer])/self.deltaLongitude
        self.flow1 = self.deltaT*self.flowSpeed*(self.deltaHeight1 - self.maximumDeltaHeight[iLayer])/self.deltaLongitude
        self.flow2 = self.deltaT*self.flowSpeed*(self.deltaHeight2 - self.maximumDeltaHeight[iLayer])/self.deltaLatitude
        self.flow3 = self.deltaT*self.flowSpeed*(self.deltaHeight3 - self.maximumDeltaHeight[iLayer])/self.deltaLatitude
        self.flow4 = self.deltaT*self.flowSpeed*(self.deltaHeight5 - np.sqrt(2)*self.maximumDeltaHeight[iLayer])/np.sqrt(self.deltaLatitude**2 + self.deltaLongitude**2)
        self.flow5 = self.deltaT*self.flowSpeed*(self.deltaHeight4 - np.sqrt(2)*self.maximumDeltaHeight[iLayer])/np.sqrt(self.deltaLatitude**2 + self.deltaLongitude**2)
        self.flow6 = self.deltaT*self.flowSpeed*(self.deltaHeight7 - np.sqrt(2)*self.maximumDeltaHeight[iLayer])/np.sqrt(self.deltaLatitude**2 + self.deltaLongitude**2)
        self.flow7 = self.deltaT*self.flowSpeed*(self.deltaHeight6 - np.sqrt(2)*self.maximumDeltaHeight[iLayer])/np.sqrt(self.deltaLatitude**2 + self.deltaLongitude**2)
        self.flow0[self.flow0 < 0] = 0
        self.flow1[self.flow1 < 0] = 0
        self.flow2[self.flow2 < 0] = 0
        self.flow3[self.flow3 < 0] = 0
    ...
    def ScaleFlow(self, iLayer):
        ...
        self.flow[:, :, 7] *= flowScaling
        '''
        flowScaling = self.terrain[:, :, iLayer]*self.deltaLongitude*self.deltaLatitude / ((self.flow0 + self.flow1 + self.flow2 + self.flow3 + self.flow4 + self.flow5 + self.flow6 + self.flow7 + 0.001) * self.deltaT)
        flowScaling[flowScaling > 1] = 1
    ...
    def UpdateTerrainHeight(self, iLayer):
        #self.terrain[:, :, iLayer] -= self.deltaT*np.sum(self.flow, axis=2)
        self.terrain[:, :, iLayer] -= self.deltaT * (self.flow0 + self.flow1 + self.flow2 + self.flow3 + self.flow4 + self.flow5 + self.flow6 + self.flow7)/(self.deltaLatitude*self.deltaLongitude)
        self.terrain[:, :, iLayer] += self.deltaT*(self.inFlowLeft + self.inFlowRight + self.inFlowTop + self.inFlowBottom + self.inFlowTopLeft + self.inFlowBottomRight + self.inFlowBottomLeft + self.inFlowTopRight)/(self.deltaLatitude*self.deltaLongitude)
...
class ThermalWeatheringScaled():
    ...
    def Weather(self, amount):
        ...
        '''
        amountToWeather = amount * np.ones(self.layerShape)
        #amountToWeather = self.deltaLatitude * self.deltaLongitude * amount * np.ones(self.layerShape)
        for iLayer in np.linspace(self.numberOfLayers-1, 0, self.numberOfLayers, dtype=int):
            if iLayer == self.numberOfLayers-1:
                amountToWeather -= self.terrain[:, :, iLayer]
                amountToWeather[amountToWeather < 0] = 0
                ...
                self.terrain[:, :, iLayer] -= self.weatheringRate[iLayer]*amountToWeather
                self.terrain[:, :, iLayer+1] += 2*self.weatheringRate[iLayer]*amountToWeather
